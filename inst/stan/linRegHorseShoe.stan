...
parameters {
  ...
  real<upper = 0> logsigma; //the standard deviation
  vector[K-K1] zvalues;
...
if(logitR){
  yL ~ bernoulli_logit(append_col(X, X2) * betaAll);
} else {
...
generated quantities{
  vector[N] log_lik;
  if(logitR){
    for (n in 1:N) log_lik[n] = bernoulli_logit_lpmf(yL[n] | append_col(X[n, ], X2[n, ]) * betaAll);
  } else {
